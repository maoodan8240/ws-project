...
public class _DayChanged extends ReentrantReadWriteLockSupport implements DayChanged {
    ...
    public void setDayChangedStr() {
        try {
            lockWrite();
            logger.info("切日前-->系统的切日时间为：" + curDayChangedStr);
            String day = getDayChangedStrByDate(new Date());
            _setCurDayStr(day);
            logger.info("切日后-->系统的切日时间为：" + curDayChangedStr);
        } finally {
            unlockWrite();
        }
    }
    ...
    private void _init() {
        try {
            ...
            if (!StringUtils.isEmpty(str)) {
                ...
                throw new RuntimeException("读取tasklist.cron4j文件的tellDayChanged配置为空！");
            }
        } finally {
            unlockWrite();
        }
    ...
    private String _getTellDayChangedCron4j() {
        ...
        Object rs = FileReader.read(reader);
        if (rs == null) {
            throw new RuntimeException("读取tasklist.cron4j文件内容异常！");
        }
        return rs.toString();
    }
