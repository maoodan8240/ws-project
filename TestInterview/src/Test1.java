...
public class Test1 {
    ...
    public static List<Path> deleteFile(LocalDate localDate, Path path) {
        ...
        listFile(file, deleteDate, deleteList);
    ...
    public static List<Path> listFile(File file, LocalDate deleteDate, List<Path> pathList) {
        if (file != null) {
            if (file.isDirectory()) {
                ...
                if (files.length == 0) {
                    ...
                    for (File file1 : files) {
                        listFile(file1, deleteDate, pathList);
                    }
                }
            } else {
